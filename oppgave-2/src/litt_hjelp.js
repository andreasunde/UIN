...
const handleSearch = () => {
  ...
  if (searchName) {
    ...
    const searchResult = users.find(
      (user) => searchName.toLowerCase() === user.name.toLowerCase()
    );
    ...
    if (searchResult) {
      ...
      userUl.innerHTML =
        "Vi fant desverre ingen ved dette navnet: " + searchName;
    }
  } else {
...
const handleFilter = () => {
  ...
  if (filterValue && typeof filterValue === "number") {
    ...
    if (filterResult && filterResult.length > 0) {
      ...
      // TODO: Oppdatere grensesnittet med userUl.innerHTML og en passende tekst når vi ikke finner noe
      userUl.innerHTML = "Vi fant ingen over denne alderen " + filterValue;
    }
  } else {
